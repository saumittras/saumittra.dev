...
export default function BlogPage() {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const { data } = await fetchApi("/blog-posts?published=true");
        if (data?.data) {
                ...
                >
                  {blog.imageUrl ? (
                    ...
                    <div className="h-48 bg-slate-100 dark:bg-slate-800 p-6 flex items-center justify-center">
                      <span className="text-4xl text-slate-300 dark:text-slate-700 font-bold tracking-tighter mix-blend-overlay">
                        BLOG
                      </span>
                    </div>
                  )}
