...
export default function RecentBlogs() {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const { data } = await fetchApi("/blogPosts?limit=3&published=true");
        if (data?.data) {
             setBlogs(Array.isArray(data.data) ? data.data.slice(0,3) : data.data.data?.slice(0,3) || []);
        }
      } finally {
        setLoading(false);
      }
  ...
  if (loading) return <div className="h-40 flex items-center justify-center text-slate-400">Loading blogs...</div>;
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-end justify-between">
        ...
        <Link href="/blog" className="text-blue-600 hover:text-blue-700 hidden sm:flex items-center space-x-1 font-medium pb-1">
          <span>Read All</span>
               ...
               <div className="p-6 flex-1 flex flex-col justify-between">
                  <div>
                      <div className="flex items-center space-x-3 mb-3 text-xs text-slate-500">
                          <span className="bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-md">{blog.category}</span>
                          <span className="flex items-center space-x-1"><Calendar size={12}/> <span>{new Date(blog.createdAt).toLocaleDateString()}</span></span>
                      </div>
                      <h3 className="text-xl font-bold mb-3 group-hover:text-blue-600 transition-colors line-clamp-2">{blog.title}</h3>
                      <p className="text-slate-500 text-sm line-clamp-3 mb-4">{blog.excerpt || "Read more about this topic inside the post..."}</p>
                  </div>
      ...
      <Link href="/blog" className="sm:hidden text-blue-600 flex items-center justify-center space-x-1 font-medium w-full py-4 bg-blue-50 dark:bg-blue-900/20 rounded-2xl">
